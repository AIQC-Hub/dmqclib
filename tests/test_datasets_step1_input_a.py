...
class TestInputDataSetA(unittest.TestCase):
    ...
    def _get_input_data(self, file_type=None, read_file_options=None):
        ...
        if read_file_options is not None:
            ds.config.data["step_param_set"]["steps"]["input"]["read_file_options"] = read_file_options
    ...
    def test_input_file_name(self):
        ...
        ds = InputDataSetA("NRT_BO_001", self.config)
        self.assertEqual(
            "/path/to/input_1/input_folder_1/nrt_cora_bo_test.parquet", str(ds.input_file_name)
        )
    ...
    def test_read_input_data_with_extra_options(self):
        ...
        df = self._get_input_data(file_type="parquet", read_file_options={"n_rows": 100})
