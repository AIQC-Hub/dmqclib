...
class TestSelectDataSetA(unittest.TestCase):
    ...
    def test_init_valid_dataset_name(self):
        ...
        ds = SummaryDataSetA("NRT_BO_001", self.config)
    ...
    def test_init_invalid_dataset_name(self):
        ...
        with self.assertRaises(ValueError):
            SummaryDataSetA("NON_EXISTENT_LABEL", self.config)
    ...
    def test_default_output_file_name(self):
        ...
        config_file_path = str(
            Path(__file__).resolve().parent / "data" / "config" / "test_dataset_002.yaml"
        )
        config = DataSetConfig(config_file_path)

        ds = SummaryDataSetA("NRT_BO_001", config)
        self.assertEqual(
    ...
    def test_input_data(self):
        ...
        ds = SummaryDataSetA(
            "NRT_BO_001", self.config, input_data=self.ds.input_data
        )
