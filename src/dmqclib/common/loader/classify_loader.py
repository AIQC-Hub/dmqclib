...
def load_classify_step6_classify_dataset(
    ...
    """
    Instantiate a :class:`BuildModelBase`-derived class based on the configuration.

    Specifically:

    1. Fetches the class name from the config via :meth:`DataSetConfig.get_base_class("classify")`.
    2. Looks up the class in :data:`CLASSIFY_CLASSIFY_REGISTRY`.
    ...
    :return: An instance of a class derived from :class:`BuildModelBase`.
...
def load_classify_step7_concat_dataset(
    ...
    """
    Instantiate a :class:`ConcatDatasetsBase`-derived class based on the configuration.

    Specifically:

    1. Fetches the class name from the config via :meth:`DataSetConfig.get_base_class("concat")`.
    2. Looks up the class in :data:`CLASSIFY_CONCAT_REGISTRY`.
    3. Instantiates and returns the class, optionally with various intermediate datasets.
    ...
    :return: An instance of a class derived from :class:`ConcatDatasetsBase`.
    :rtype: ConcatDatasetsBase
    """
